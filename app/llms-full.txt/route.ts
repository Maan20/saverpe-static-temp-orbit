...
export function GET() {
  const parts = [
    ...
    "## Brands",
    ...brands.map((b) => [`### ${b.name}`, `URL: ${absoluteUrl(`/brands/${b.slug}`)}`, `Category: ${getCategory(b.category)?.name}`, `Value: ${priceSummary(b.price)}${b.price.denominations?.length ? ` (denominations: ${b.price.denominations.map(formatInr).join(", ")})` : ""}`, b.expiry ? `Validity: ${b.expiry}` : ""].filter(Boolean).join("\n")),
    "## Insights articles",
